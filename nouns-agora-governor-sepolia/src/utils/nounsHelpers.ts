// Copy and paste from the Nouns helpers
// https://github.com/nounsDAO/nouns-monorepo/tree/master/packages/nouns-subgraph/src/utils
...
export function getOrCreateDelegateWithNullOption(
  ...
  if (delegate == null && createIfNotFound) {
    ...
    delegate.numberDroposalVotes = 0;
...
export function getOrCreateProposalCandidateVersion(
  id: string
): ProposalCandidateVersion {
...
export function getOrCreateProposalCandidateContent(
  id: string
): ProposalCandidateContent {
...
export function getOrCreateProposalCandidateSignature(
  id: string
): ProposalCandidateSignature {
 ...
 */
export function calcEncodedProposalHash(
  proposal: Proposal,
  isUpdate: boolean
): Bytes {
  ...
  for (let i = 0; i < proposal.calldatas!.length; i++) {
    calldatasHashes = calldatasHashes.concat(
      keccak256Bytes(proposal.calldatas![i])
    );
  }
  ...
  params.push(
    ethereum.Value.fromAddress(Address.fromString(proposal.proposer))
  );
  ...
  params.push(
    ethereum.Value.fromFixedBytes(
      keccak256Bytes(Bytes.fromUTF8(proposal.description))
    )
  );
